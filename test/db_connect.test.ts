...
describe("DB Connection Test", () => {
  test("should connect to the database and check version", async () => {
    ...
    expect(consoleLogSpy).toHaveBeenNthCalledWith(5, expect.stringMatching(/PostgreSQL 1\d+/)); // version string
    expect(consoleLogSpy).toHaveBeenNthCalledWith(6, "✅ Test completed in", expect.any(Number), "ms");
  ...
  test("should handle errors", async () => {
    ...
    // Assertions
    expect(consoleErrorSpy).toHaveBeenNthCalledWith(1, "❌ Test failed");
