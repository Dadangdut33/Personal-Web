...
import { IconSearch, IconEdit, IconTrash, IconLetterA, IconDeviceWatch } from "@tabler/icons";
...
export const User: NextPage<IDashboardProps> = (props) => {
	...
	const searchAllHelper = (item: IUser, query: string) => {
		return item.username.toLowerCase().includes(query.toLowerCase()) || formatDateWithTz(item.createdAt, tz).toLowerCase().includes(query.toLowerCase());
	};

	const isSearching = () => {
		// not on setting page and is actually searching and data all is fetched
		return tabIndex !== 2 && dataAllPage.length > 0 && (searchAll !== "" || searchUsername !== "" || searchCreatedAt !== "");
	};
